...
const CardShop = props => {
  ...
  return (
    <React.Fragment>
      <Card className="tw-h-20 bg-white tw-relative">
        <div className="tw-grid tw-grid-cols-3 tw-gap-2">
          ...
          <div className="tw-flex tw-col-span-2 tw-justify-between tw-flex-col text-left tw-h-full p-3 bg-white">
            ...
            <p className="mb-3">Total sales: {currency.format(shop.price)}</p>
        ...
        <span className="tw-absolute tw-top-3 tw-right-3">
          {" "}
          {shop.status ? (
            <span className="tw-text-green-500 tw-bg-green-100 tw-rounded-full tw-px-3 tw-py-1 tw-text-xs">Active</span>
          ) : (
            <span className="tw-text-red-500  tw-bg-red-100 tw-rounded-full tw-px-3 tw-py-1 tw-text-xs">Inactive</span>
          )}
        </span>
      </Card>
    </React.Fragment>
